...
class Extension {
  get_unfullscreen_monitor() {
    let secondaryMonitor = this._settings.get_int("secondary-monitor");
    if (
      secondaryMonitor === -1 ||
      secondaryMonitor >= LM.monitors.length ||
      LM.monitors[secondaryMonitor].inFullscreen
    ) {
      ...
      return LM.monitors[secondaryMonitor];
    }
    return null; // Return null if no valid monitor is found
  }
  ...
  patch_updateState() {
    const patches = [
      {
        from: "Main.layoutManager.primaryMonitor.",
        to: "Main.layoutManager.monitors[this._constraint.index].",
      },
  ...
  patch_getDraggableWindowForPosition() {
    ...
    Main.panel._getDraggableWindowForPosition = this.patch_function(
      func,
      patches
    );
  }
    ...
    if (
      ...
      if (!extension.stateObj._rebuild) {
        extension.stateObj._rebuild = function () {
          ...
          this.TrayIcons = new extension.imports.extension.TrayIconsClass(
            this._settings
          );
          this._setTrayMargin();
          this._setIconSize();
  ...
  enable() {
    ...
    this._original_getDraggableWindowForPosition =
      Main.panel._getDraggableWindowForPosition;
    this._settings = ExtensionUtils.getSettings();
    this._panel_monitor_index = LM.primaryIndex;
    this._on_fullscreen = Display.connect(
      "in-fullscreen-changed",
      this.fullscreen_changed.bind(this)
    );
